#!/usr/bin/env python3
"""python-afl harness for ``cidrize.cidrize`` (IPv6-shaped inputs).

Logic is identical to ``fuzz_cidrize_ipv4_python_afl.py`` — the IPv4/IPv6
split is seed-driven, not code-driven (seeds under
``benchmarks/python_afl/seeds/cidrize_ipv6/``). Two files are kept so AFL
output directories map 1:1 to the 5 run configurations in the team's
benchmark matrix.

Oracle: self-consistency + round-trip (``cidrize`` has no safe external
oracle for ranges/wildcards). See the IPv4 variant for details.
"""
